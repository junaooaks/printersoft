...
<?php
if (basename($_SERVER["PHP_SELF"]) == "acessorio.sql.php") {
...
if ($codExcluir<>''){
	...
	$sql = $pdo->prepare("DELETE FROM acessorio WHERE idAcessorio = :cod");
...
if (($flag == 2) and ($codAlterar<>'')){
	...
	$up = $pdo->prepare("UPDATE acessorio SET descricao = :grupo WHERE idAcessorio = :id");
...
$sql = $pdo->prepare("INSERT INTO acessorio (descricao) VALUES (?)");
...
exit();
	...
	if ($codAlterar <>''){
		...
		$sele = $pdo->prepare("SELECT * FROM acessorio WHERE idAcessorio = :id");
		$sele->execute($filtro);
		
		while($row = $sele->fetch())
        {
		    	$id	  = utf8_encode($row['idAcessorio']);
...
/******pegar o resultado ****************/
$sele = $pdo->prepare("SELECT * FROM acessorio");
$sele->execute();
