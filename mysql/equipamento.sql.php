...
<?php
if (basename($_SERVER["PHP_SELF"]) == "equipamento.sql.php") {
...
if ($codExcluir<>''){
	...
	$sql = $pdo->prepare("DELETE FROM equipamento WHERE idEquipamento = :cod");
...
if (($flag == 2) and ($codAlterar<>'')){
	...
	$up = $pdo->prepare("UPDATE equipamento SET descricao = :grupo WHERE idEquipamento = :id");
...
$sql = $pdo->prepare("INSERT INTO equipamento (descricao) VALUES (?)");
...
exit();
	...
	if ($codAlterar <>''){
		...
		$sele = $pdo->prepare("SELECT * FROM equipamento WHERE idEquipamento = :id");
		$sele->execute($filtro);
		
		while($row = $sele->fetch())
        {
		    $id		  = utf8_encode($row['idEquipamento']);
...
/******pegar o resultado ****************/
$sele = $pdo->prepare("SELECT * FROM equipamento");
$sele->execute();
