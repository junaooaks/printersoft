...
<?php
if (basename($_SERVER["PHP_SELF"]) == "garantia.sql.php") {
...
if ($codExcluir<>''){
	...
	$sql = $pdo->prepare("DELETE FROM garantia WHERE idGarantia = :cod");
...
if (($flag == 2) and ($codAlterar<>'')){
	...
	$up = $pdo->prepare("UPDATE garantia SET descricao = :grupo WHERE idGarantia = :id");
...
$sql = $pdo->prepare("INSERT INTO garantia (descricao) VALUES (?)");
...
exit();
	...
	if ($codAlterar <>''){
		...
		$sele = $pdo->prepare("SELECT * FROM garantia WHERE idGarantia = :id");
		$sele->execute($filtro);
		
		while($row = $sele->fetch())
        {
		    $id		  = utf8_encode($row['idGarantia']);
...
/******pegar o resultado ****************/
$sele = $pdo->prepare("SELECT * FROM garantia");
$sele->execute();
