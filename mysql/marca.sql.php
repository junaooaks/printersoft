...
<?php
if (basename($_SERVER["PHP_SELF"]) == "marca.sql.php") {
...
if ($codExcluir<>''){
	...
	$sql = $pdo->prepare("DELETE FROM marca WHERE idMarca = :cod");
...
if (($flag == 2) and ($codAlterar<>'')){
	...
	$up = $pdo->prepare("UPDATE marca SET descricao = :grupo WHERE idMarca = :id");
...
$sql = $pdo->prepare("INSERT INTO marca (descricao) VALUES (?)");
...
exit();
	...
	if ($codAlterar <>''){
		...
		$sele = $pdo->prepare("SELECT * FROM marca WHERE idMarca = :id");
		$sele->execute($filtro);
...
/******pegar o resultado ****************/
$sele = $pdo->prepare("SELECT * FROM marca");
$sele->execute();
		
				
 while($row = $sele->fetch())
        {
		    $id		  = utf8_encode($row['idMarca']);
			$grupo  = utf8_encode($row['descricao']);
