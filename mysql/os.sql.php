...
$s= $pdo->prepare("INSERT INTO laudo (codOs, codCliente, valor, prazo, disponibilidade, laudo, hora, status, obs) 
			  VALUES (?,?,?,?,?,?,?,?,?)");
		 
$s->execute(array ($os, $cliente, $valor, $prazo, $dis, $laudo, $time, $radio, $obs));
...
if (($flag == 2) and ($codAlterar<>'')){
	...
	//consultar se numero do protocolo ja existe
	$filtro = array('proto'=>$protocolo);
		$sele = $pdo->prepare("SELECT protocolo, idOs FROM os WHERE protocolo = :proto");
		$sele->execute($filtro);
		...
		//comparar se numero pertence a os
			//echo "os $osp - alterar $codAlterar - con proto $pro - form proto $protocolo"; exit();
			/*
			if (($osp<>$codAlterar)and($pro==$protocolo)){
				
				echo "<script type='text/javascript'> window.alert('NUMERO DE PROTOCOLO JA EXISTENTE');</script>";
				echo "<meta http-equiv='refresh' content='0;URL=index.php?codcliente=$codCliente & codos=$codAlterar'>";
				exit();	
				}*/
...
/*
echo "codcli $codCliente, proto $protocolo, datac $dataC, equip $equipamento, marca $marca, atend $atendimento, acess $acessorio, tecni $tecnico, statu $status, previs $previsao, modelo $modelo, patrim $patrimonio, seri $serie, setor $setor, adici $adicionais, dia $diag"; exit();
*/


$sqll= $pdo->prepare("INSERT INTO os (codCliente, protocolo, data, codEquipamento, codMarca, codAtendimento, codAcessorio, codTecnico, codStatus, previsao, modelo, patrimonio, serie, setor, diagnostico, situacao) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
		 
$sqll->execute(array ($codCliente, $protocolo, $dataC, $equipamento, $marca, $atendimento, $acessorio, $tecnico, $status, $previsao, $modelo, $patrimonio, $serie, $setor, $adicionais, $diag));
