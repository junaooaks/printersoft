...
<?php
if (basename($_SERVER["PHP_SELF"]) == "servico.sql.php") {
...
if ($codExcluir<>''){
	...
	$sql = $pdo->prepare("DELETE FROM servico WHERE idServico = :cod");
...
if (($flag == 2) and ($codAlterar<>'')){
	...
	$up = $pdo->prepare("UPDATE servico SET descricao = :grupo WHERE idServico = :id");
...
$sql = $pdo->prepare("INSERT INTO servico (descricao) VALUES (?)");
...
exit();
	...
	if ($codAlterar <>''){
		...
		$sele = $pdo->prepare("SELECT * FROM servico WHERE idServico = :id");
		$sele->execute($filtro);
		
		while($row = $sele->fetch())
        {
		    $id		  = utf8_encode($row['idServico']);
...
/******pegar o resultado ****************/
$sele = $pdo->prepare("SELECT * FROM servico");
$sele->execute();
